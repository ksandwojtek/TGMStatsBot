...
class Statss(commands.Cog):
    ...
    guild_ids = GlobalVariables.config['bot']['guilds']
    ...
    @cog_ext.cog_slash(name='Stats', description='Displays player stats on team games', guild_ids=guild_ids,
                 options=options)
        ...
        if ctx.channel.id in self.global_variables.config['bot']['channels']:
            async with ctx.typing():
                ...
                async with aiohttp.ClientSession(connector=connector) as cs:
                    ...
                    ################################################################
                    async with cs.get('https://api.pvparca.de/mc/player/' + mc_name, ) as r:
                        res = await r.json()
